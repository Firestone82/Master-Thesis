...
Namísto jednoho attention mechanismu používají Transformery \textit{multi-head attention}, tedy $h$ paralelních attention hlav zpracovává vstup v různých pod prostorech, přičemž se každá hlava může specializovat na jiný typ závislosti (syntaktický vztah, sémantická vazba, vzdálenostní vzor apod.).
...
Tato vrstva slouží jako adresovatelná paměť uchovávající faktické znalosti a reziduální spojení s vrstvou normalizací stabilizující data tréninku.
...
\paragraph{Předtrénování (pre-training).}
...
\paragraph{Instrukční dolaďování (supervised fine-tuning, SFT).}
...
\TODO{QUESTION: Stejně jako u trénování, zmínit toto až u implementace?}
