...
Velký jazykový model (Large Language Model, LLM) je třída modelů strojového učení určená k modelování pravděpodobnostního rozdělení přirozeného jazyka~\cite{redhat2024llm,jelodar2025llmsourcecode}.
...
Nejznámějším příkladem je chatbot ELIZA z roku 1966, který simuloval psychoterapeuta pomocí jednoduchých vzorů přepisování vstupních vět~\cite{weizenbaum1966eliza}.
ELIZA přes svou primitivnost vyvolávala v uživatelích dojem skutečného porozumění -- jev, který Joseph Weizenbaum nazval \textit{ELIZA effect}~\cite{weizenbaum1966eliza}.
...
Modely Word2Vec (2013)~\cite{mikolov2013word2vec} a GloVe (2014)~\cite{pennington2014glove} ukázaly, že lze natrénovat reprezentace zachycující sémantické vztahy: král, můž, žena se blíží slovu královna.

Druhou inovací jsou rekurentní neuronové sítě (RNN) a zejména jejich varianta LSTM (Long Short-Term Memory)~\cite{hochreiter1997lstm}.
...
Zásadní průlom nastal v roce 2017 s publikací práce \textit{Attention is All You Need} od výzkumníků z Google Brain~\cite{vaswani2017attention}.
...
GPT-3 (2020, 175 miliard parametrů)~\cite{brown2020gpt3} ukázal, že se škálováním modelů objevují kvalitativně nové schopnosti.
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{lll}
            ...
            2017--2020    & Transformer, předtrénování      & BERT (2018)~\cite{devlin2019bert}, GPT-2 (2019) \\
...
Přičemž pro dosažení optimálního výkonu by počet trénovacích tokenů měl být přibližně dvacetinásobkem počtu parametrů~\cite{hoffmann2022chinchilla}.
