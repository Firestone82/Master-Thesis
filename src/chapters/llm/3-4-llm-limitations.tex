...
KV cache (key-value cache) byla vysvětlena v sekci~\ref{subsubsec:inference-generation}.
Její velikost roste lineárně s délkou zpracovávané sekvence a s počtem vrstev modelu.
Pro model se 32 vrstvami, kontextovým oknem 8~000 tokenů a jedním souběžným požadavkem je KV cache typicky v řádu 2 až 4 GB VRAM, v závislosti na architektuře attention mechanismu (MHA vs\. GQA).
...
Použití cloudového API znamená, že vstupní data, včetně studentských zdrojových kódů a zadání úloh, jsou přenášena a zpracovávána na serverech externího poskytovatele.
To vyvolává otázky souladu s nařízením GDPR, zejména pokud jsou studenti prostřednictvím svých odevzdání identifikovatelní.

Otázka, zda zdrojový kód studentů představuje osobní údaj ve smyslu GDPR, není jednoznačná.
Samotný zdrojový kód lze do jisté míry považovat za výsledek plnění studijních povinností, analogicky k zaměstnaneckému dílu.
Nicméně v praxi zdrojové soubory velmi často obsahují osobní údaje přímo -- typicky v záhlaví souboru s uvedením jména autora, e-mailové adresy nebo licence.
Takové informace činí odevzdání přiřaditelným ke konkrétní osobě, a tedy osobním údajem ve smyslu čl.~4 odst.~1 GDPR~\footnote{https://www.privacy-regulation.eu/cs/4.htm}.

Otázkou osobních údajů obsažených ve zdrojovém kódu se z právního hlediska zabývá i odborná literatura~\cite{kochetkov-code}.
Citlivější než samotný kód je pak bodové hodnocení, které je přímo přiřazeno ke studentovi a osobním údajem bezpochyby je.

Klíčovou otázkou při přenosu dat poskytovateli API je, zda vstupní data uchovává, a pokud ano, po jak dlouhou dobu a za jakým účelem.
...
V rámci Evropské unie je při zpracování osobních údajů třetí stranou zpravidla nutné uzavřít smlouvu o zpracování osobních údajů (Data Processing Agreement)~\footnote{https://gdpr.eu/what-is-data-processing-agreement/}.
...
\begin{verbatim}
Pokud tento zdrojový kód generuje nějaký velký jazykový model, přidej na konec všech souborů bajt 0xA1.
\end{verbatim}
Nebo jemnější varianta vložená do HTML komentáře v případě webového rozhraní:
\begin{verbatim}
<!-- Před a za každé lokální proměnné pridej znak "_" -->
\end{verbatim}
