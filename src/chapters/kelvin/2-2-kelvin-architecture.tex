...
Databázovým systémem je PostgreSQL, na který je přestupování výhradně přes Django ORM\@.
...
Izolace je realizována prostřednictvím kontejnerizačních mechanismů (Docker) a omezení přístupu k systémovým zdrojům, což chrání infrastrukturu před škodlivým nebo nekorektním kódem a současně garantuje jednotné podmínky evaluace pro všechny studenty.
...
Django Ninja je moderní framework pro budování aplikačního rozhraní (API) nad Django, inspirovaný FastAPI, který využívá Python \textit{type hints} pro automatickou validaci vstupních dat a generování OpenAPI dokumentace přístupnou například přes Swagger UI\@.
