...
Každé odevzdání bylo zpracováno modelem Qwen3-Coder:30B oběma přístupy: chain-of-thought se třemi kroky analýzy a one-shot s jediným voláním modelu.
Detailní výsledky jsou v \tabref{tab:cot-vs-oneshot-detail}, naměřená spotřeba tokenů v \tabref{tab:token-usage} a souhrnné průměry v \tabref{tab:cot-vs-oneshot-summary}.
...
\begin{table}[H]
    ...
    \caption{Detailní výsledky experimentu s modelem Qwen3-Coder:30B na pěti studentských odevzdání z předmětu UPR\@. Každé odevzdání bylo analyzováno oběma strategiemi.}
...
\begin{table}[H]
    ...
    \caption{Naměřená spotřeba tokenů při analýze studentských odevzdání dle \tabref{tab:cot-vs-oneshot-detail}.}
...
Přístup one-shot je výrazně rychlejší (průměrně 15,7~s oproti 134,2~s) a spotřebovává přibližně čtyřikrát méně vstupních a desetkrát méně výstupních tokenů.
Přístup one-shot zároveň naivně nachází vyšší počet problémů (4,6 oproti 3,0); chain-of-thought záměrně filtruje falešné pozitivní nálezy v kroku kritické revize, a proto reportuje méně, avšak pečlivěji ověřených zjištění.
Z hlediska nákladů na cloudové API je vliv výběru strategie zásadní: jak ukazuje \tabref{tab:cloud-api-costs}, chain-of-thought zvyšuje cenu za jedno odevzdání přibližně čtyř až desetkrát oproti one-shot přístupu.
...
Pro cloudové API bude k dispozici volba one-shot, která výrazně snižuje cenu za odevzdání při zachování přijatelné kvality výstupů.
